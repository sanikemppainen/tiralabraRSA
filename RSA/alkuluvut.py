...
def tarkistaa_alkuluvut_miller_rabin_algoritmilla(alkuluvut, iteraatiot):
    """
    Tarkistaa onko alkuluvut listalla todennäköisesti oikeasti alkulukuja.
    Iteraatiot arvoa kasvattamalla tämä algoritmi ajetaan useempaan kertaan,
    tehden tästä varmemman sillä algoritmi on todennäköisyysperustainen ja
    voi antaa vääriä positiivisa väittämiä alkuluvusta.
...
def on_todennakoisesti_alkuluku(i, listanpituus, iteraatiot):
    ...
    Alkuluku on lukua 1 suurempi luonnollinen luku, joka ei ole jaollinen muilla
    positiivisilla kokonaisluvuilla kuin yhdellä ja itsellään. 2 on ainut parillinen alkuluku.
    ...
    # entä 3? sekin pitää laittaa trueksi heti alussa,
    # muuten iteraation mukainen looppaus ei toimi (se ei katso 3, alottaa 4)
    if i == 3:
        return True
    ...
    for k in range(iteraatiot):
        ...
        for j in range(y - 1):
            ...
            return False
        # jos tuo algoritmi ei ole laittanut i falseksi niin sitten i
        # on luultavaksi alkuluku ja palautetaan True
    return True
